...
App.TextField = App.Field.extend({
  init: function() {
      ...
      this.set('field', SC.TextField.create({
        parent: null,
        attributeBindings: ['maxlength'],
        placeholderBinding: 'parent.placeholder',
        maxlengthBinding: 'parent.maxLength',
      }));
      
      this.get('field').set('parent', this);   
    },
...
Handlebars.registerHelper('form', function(path, options) {
  var inverse = options.inverse,
          data = options.data,
          view = data.view,
          fn = options.fn,
          hash = options.hash
          field = options.contexts[0].getPath(path);
